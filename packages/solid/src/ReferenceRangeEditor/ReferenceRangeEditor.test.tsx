...
import { deepClone, UCUM } from '@medplum/core';
...
describe('ReferenceRangeEditor', () => {
  ...
  test('Set Gender Filter', async () => {
    ...
    fireEvent.click(within(screen.getByTestId('reference-range-editor')).getByText('Save'));
  ...
  test('Set Age Filter', async () => {
    ...
    fireEvent.input(ageLowInput, { target: { value: '10' } });
    fireEvent.input(ageHighInput, { target: { value: '18' } });
    fireEvent.click(screen.getByText('Save'));
  ...
  test('Set Null Filter', async () => {
    ...
    fireEvent.click(screen.getByText('Save'));
    const checkSubmitted = onSubmit.mock.calls[0][0] as ObservationDefinition;
    expect(checkSubmitted.qualifiedInterval).toBeDefined();
    expect(checkSubmitted.qualifiedInterval).toHaveLength(3);
    for (const interval of checkSubmitted.qualifiedInterval || []) {
      expect(interval).not.toHaveProperty('gender');
      expect(interval).not.toHaveProperty('context');
      expect(interval).not.toHaveProperty('category');
    }
  });
  ...
  test('Add Interval w/ filters', async () => {
    ...
    fireEvent.click(screen.getByTitle('Add Interval'));
    fireEvent.input(screen.getByTestId('range-id-4-low-value'), { target: { value: 99 } });
  ...
  test('Interval Ids', async () => {
    ...
    fireEvent.input(screen.getByTestId('range-id-67-low-value'), { target: { value: 2 } });
    fireEvent.input(screen.getByTestId('range-id-68-low-value'), { target: { value: 8 } });
  ...
  test('Update Range', async () => {
    ...
    fireEvent.input(valueInput, { target: { value: '3' } });
    fireEvent.click(screen.getByText('Save'));
  ...
  test('Update Multiple Group Filters', async () => {
    ...
    fireEvent.input(ageHighInput, { target: { value: '16' } });
    fireEvent.input(ageLowInput, { target: { value: '17' } });
    fireEvent.click(screen.getByText('Save'));
  ...
  test('Overlapping Group Filters', async () => {
    ...
    const ageLowInput = screen.getByTestId('age-group-id-2-low-value');
    fireEvent.input(ageLowInput, { target: { value: '11' } });
    fireEvent.click(screen.getByText('Save'));
